...
const removeDB = (dbName) => {
   const sqlCreateDB = `DROP DATABASE IF EXISTS ${dbName}`;

   return new Promise((resolve, reject) => {
      const connection = mysql.createConnection(initialConfig);
      try {
         console.log("Checking/dropping db...");
         connection.query(sqlCreateDB, (error, result) => {
            if (error)
               throw new Error(`Error dropping DB: ${dbName} ` + error.message);
            console.log(`DB: ${dbName} is dropped.`);
            resolve(result);
         });
      } catch (error) {
         reject(error);
...
module.exports = { createDB, removeDB };
